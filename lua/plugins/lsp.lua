...
local LSP_Keybind = function()
  ...
  vim.api.nvim_create_autocmd("LspAttach", {
    ...
    callback = function(event)
      ...
      map("n", "<leader>ld", function() diagnostic.open_float() end, { desc = "Open Float (LSP)", buffer = event.buf })
      map("n", "]d", function() diagnostic.goto_next() end, { desc = "Go Next (LSP)", buffer = event.buf })
      map("n", "[d", function() diagnostic.goto_prev() end, { desc = "Go Prev (LSP)", buffer = event.buf })
      map("n", "<leader>lc", function() lsp_Buf.code_action() end, { desc = "Action (LSP)", buffer = event.buf })
      map("n", "<leader>lr", function() lsp_Buf.references() end, { desc = "References (LSP)", buffer = event.buf })
      map("n", "<leader>ln", function() lsp_Buf.rename() end, { desc = "Rename (LSP)", buffer = event.buf })
    end,
  })
end
